...
export default function Header() {
  ...
  return (
    <>
      {/* ── GOLDEN TOP BAR (desktop only) ── */}
      <div
        className="fixed top-0 left-0 right-0 z-50 hidden lg:block"
        style={{ backgroundColor: '#E8B92C', borderBottom: '1px solid rgba(0,0,0,0.12)' }}
      >
        <div className="max-w-[1600px] mx-auto px-12 flex items-center justify-between" style={{ height: '34px' }}>
          <p className="text-stone-900 text-xs font-bold tracking-widest uppercase">
            Building Excellence Across the UAE
          </p>
          <div className="flex items-center gap-4">
            <span className="text-stone-900/50 text-xs font-semibold uppercase tracking-wider">Follow Us:</span>
            {socialLinks.map((social) => (
              <a
                ...
                className="text-stone-900 hover:opacity-60 transition-all duration-200 hover:scale-125"
              >
                {social.icon}
              </a>
            ))}
      ...
      {/* ── MAIN NAV ── */}
      <nav
        className="fixed left-0 right-0 z-40 lg:top-[34px] top-0"
        style={{
      ...
      >
        <div className="max-w-[1600px] mx-auto px-6 lg:px-12 py-3 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            <img src={IMG.logo} alt="Royal Track Logo" className="h-28 w-auto object-contain" />
          ...
          <div className="hidden lg:flex items-center gap-8 text-white">
            ...
            <Link
              href="/#contact"
              className="flex items-center gap-2 px-6 py-3 bg-[#E8B92C] hover:bg-[#C9831A] text-stone-900 transition-all duration-300 text-sm font-extrabold uppercase tracking-wider rounded-full shadow-lg hover:shadow-xl hover:scale-105"
            >
              GET QUOTE →
            </Link>
          </div>
